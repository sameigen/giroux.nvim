...
| `roster.lua` | `:Giroux` — the board. One line per session, attention-sorted, live off a monitor subscription. Spawned subagents (Agent/Task) nest as indented `⤷` rows under their parent (type + task from the `.meta.json` sidecar), attention-sorted and capped; their state rolls up into the parent's group header. Observe-only — steering acts on the parent. |
...
| `tmuxctl.lua` | tmux correlation + control. Maps a transcript to its tmux session by PROOF: `claude agents --json` gives sessionId→pid, `tmux list-panes` gives pane_pid, and a ps pid/ppid snapshot links them by ancestry — exact even with N parallel sessions in one repo. Nodes without `claude agents` fall back to cwd-slug matching only when unambiguous (a single candidate). Renames sessions to the transcript's ai-title (`giroux/t/<slug>-<sid4>`, sid-suffixed so same-titled sessions can't collide; bookkeeping only on rename success; never fighting manual names). All tmux `-t` targets are `=`-pinned (tmux prefix-matches otherwise). Caches per-node snapshots with in-flight coalescing. `scripts/claude-wrapper.zsh` is the capture side: interactive `claude` lands in `tmux` with minimal status, mouse, set-titles. |
